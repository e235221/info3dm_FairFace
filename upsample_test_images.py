...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('test_upsampling.log')
        ]
    )
...
def upsample_test_images(input_dir, output_dir, target_size=(512, 512), quality=95, 
                        max_files=None, overwrite=True):
    """
    testフォルダ内の画像をアップサンプリングします（解像度を上げます）。
    
    Args:
        input_dir (str): 入力画像ディレクトリ（test）
...
def main():
    parser = argparse.ArgumentParser(description='testフォルダの画像をアップサンプリング')
    parser.add_argument('--input_dir', default='test', help='入力画像ディレクトリ')
    parser.add_argument('--output_dir', default='test_upsampled', help='出力画像ディレクトリ')
