...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('detected_faces_downsampling.log')
        ]
    )
...
def downsample_detected_faces(input_dir, output_dir, target_size=(224, 224), quality=85, 
                             max_files=None, overwrite=True):
    """
    detected_facesフォルダ内の画像をダウンサンプリングします。
    ...
    for i, filename in enumerate(image_files, 1):
        ...
        if downsample_image(input_path, output_path, target_size, quality):
            processed_count += 1
        else:
            error_count += 1
...
def main():
    parser = argparse.ArgumentParser(description='detected_facesフォルダの画像をダウンサンプリング')
    parser.add_argument('--input_dir', default='detected_faces', help='入力画像ディレクトリ')
    parser.add_argument('--output_dir', default='detected_faces_downsampled', help='出力画像ディレクトリ')
